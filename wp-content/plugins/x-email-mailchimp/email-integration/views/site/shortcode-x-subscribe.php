...
// "invisible" [content_band] shortcode, which simply acts as a container.
// Otherwise, allow users to tap into [content_band] options via the plugin.
//

if ( $custom_styling == 'No' ) {

  $shortcode = do_shortcode( '[content_band ' . $class . ' padding_top="0" padding_bottom="0"]' . $form . '[/content_band]' );
  ...
  $border = ' border="' . strtolower( $border ) . '"';
  ...
  $shortcode = do_shortcode( '[content_band ' . $class . ' style="padding: ' . $padding . ';"' . $margin . $border . $inner . $bg . ']' . $form . '[/content_band]' );
